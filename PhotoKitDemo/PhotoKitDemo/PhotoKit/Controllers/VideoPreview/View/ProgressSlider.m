...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == ProgressSliderHorizontal ? _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == ProgressSliderHorizontal ? (self.frame.size.height - _lineWidth) / 2 : _circleRadius);//起点
    
    CGFloat endLineX = (_direction == ProgressSliderHorizontal ? self.frame.size.width - _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == ProgressSliderHorizontal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- _circleRadius);//终点
    ...
    CGFloat progressLineX = (_direction == ProgressSliderHorizontal ? MAX(_circleRadius, (_progressPercent * self.frame.size.width - _circleRadius)) : startLineX);
    
    CGFloat progressLineY = (_direction == ProgressSliderHorizontal ? startLineY : MAX(_circleRadius, (_progressPercent * self.frame.size.height - _circleRadius)));
    ...
    CGFloat slidedLineX = (_direction == ProgressSliderHorizontal ? MAX(_circleRadius, (_sliderPercent * (self.frame.size.width - 2*_circleRadius) + _circleRadius)) : startLineX);
    
    CGFloat slidedLineY = (_direction == ProgressSliderHorizontal ? startLineY : MAX(_circleRadius, (_sliderPercent * self.frame.size.height - _circleRadius)));
    ...
    CGFloat circleX = (_direction == ProgressSliderHorizontal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth ) : startLineX);
    CGFloat circleY = (_direction == ProgressSliderHorizontal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.sliderPercent = (_direction == ProgressSliderHorizontal ? touchPoint.x : touchPoint.y) / (_direction == ProgressSliderHorizontal ? self.frame.size.width : self.frame.size.height);
}
